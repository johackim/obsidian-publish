const links = JSON.parse(process.env.navigation);
...
export default {
    ...
    a: ({ children, href }) => {
        ...
        const isNotExist = !isExternal && !links.some((link) => `/${link.permalink}` === href || link.fileName === children);

        return (
            <a
                href={href}
                {...(isExternal ? { target: '_blank', rel: 'noopener noreferrer', className: 'external-link' } : {})}
                {...(isNotExist ? { className: 'internal-link is-unresolved' } : {})}
            >
                {children}
            </a>
        );
